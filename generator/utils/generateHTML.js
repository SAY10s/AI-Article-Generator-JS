...
export function generateHTML(dataArray) {
  let html = "";
  html += `<div class="my-4 sm:w-9/12 md:w-1/3 border-2 border-color-[#d9d9d9]" style="padding: 20px;">
  <ol>`;
  let contentsList = dataArray.map((item, index) => {
    return `<li><a href="#a${index + 1}">${item.header}</a></li>`;
  });
  html += contentsList.join("");
  html += `</ol></div>`;
  let articles = dataArray.map((item, index) => {
